...
public extension Array where Element == FixVec {
    ...
    private func createEdges() -> [ABEdge] {
        var edges = [ABEdge](repeating: .zero, count: count)
        var a = self[count - 1]
        for i in 0..<count {
            let b = self[i]
            if a.bitPack > b.bitPack {
                edges[i] = ABEdge(e0: b, e1: a)
            } else {
                edges[i] = ABEdge(e0: a, e1: b)
            }
            a = b
        }
        return edges
        ...
        while !queue.isEmpty {
            ...
            for scanIndex in 0..<scanList.count {
                ...
                case .pure:
                    ...
                    let thisLt = ABEdge(e0: thisEdge.e0, e1: x)
                    let thisRt = ABEdge(e0: x, e1: thisEdge.e1)
                    
                    isAnyBend = isAnyBend || Triangle.isNotLine(p0: scanEdge.e0, p1: scanEdge.e1, p2: x)
                    
                    let scanLt = ABEdge(e0: scanEdge.e0, e1: x)
                    let scanRt = ABEdge(e0: x, e1: scanEdge.e1)
