...
public extension Array where Element == FixVec {
    ...
    func isPointInsideConvexPolygon(point: FixVec) -> Bool {
        ...
        if Self.orientation(a: p0, b: p1, c: point) < 0 {
            return false
        }
        ...
        while high - low > 1 {
            let mid = (low + high) >> 1
            let pm = self[mid]
            if Self.orientation(a: p0, b: pm, c: point) < 0 {
                high = mid
            } else {
                low = mid
            }
        ...
        return Self.isTriangleContain(a: p0, b: self[low], c: self[high], p: point)
    }
